...
from text_editor.models import (
...
class ConsoleEditor(object):
    def __init__(self):
        ...
        self._console_menu: ConsoleMenu = ConsoleMenu(
            savers={
                ...
                "cloud": GoogleDriveFileManager(
                    "manifest-bit-454816-m5-fd109a3c8c1f.json"
                ),
            },
    ...
    def _set_key_bindings(self, kb: KeyBindings):
        ...
        def increase_font(event):
            ...
            event.app.layout.visible_windows[0].style = (
                f"class:{self.__editor.settings.font_size}"
            )
            event.app.layout.visible_windows[1].style = (
                f"class:{self.__editor.settings.font_size}"
            )
        ...
        def apply_bold(event):
            ...
            if select_indexes:
                self.__editor.apply_style(
                    select_indexes[0], select_indexes[1], bold=True
                )
                self._update_buffers()
        ...
        def apply_italic(event):
            ...
            if select_indexes:
                self.__editor.apply_style(
                    select_indexes[0], select_indexes[1], italic=True
                )
                self._update_buffers()
        ...
        def cut(event):
            ...
            if select_indexes:
                pyperclip.copy(
                    event.current_buffer.text[select_indexes[0] : select_indexes[1] + 1]
                )
        ...
        def apply_strikethrough(event):
            ...
            if select_indexes:
                self.__editor.apply_style(
                    select_indexes[0], select_indexes[1], strikethrough=True
                )
                self._update_buffers()
    ...
    def _on_text_changed(self, new_text: str, cursor_position: int):
        ...
        if new_len > old_len:
            cursor_position -= 1
            self.__editor.insert_text(
                new_text[cursor_position : cursor_position + new_len - old_len],
                cursor_position,
            )
        elif old_len == new_len:
            return
        else:
            self.__editor.erase_text(
                cursor_position, cursor_position + old_len - new_len - 1
            )
    ...
    def _run_editor_space(self):
        self.buffer = buffer = Buffer(
            on_text_changed=lambda buff: self._on_text_changed(
                buff.text, buffer.cursor_position
            )
        )
        ...
        notification_window = Window(
            content=BufferControl(buffer=self.notification_buffer),
            height=5,
