...
class TextComponent(ITextComponent):
    ...
    def to_dict(self) -> dict:
        return {"type": self.__class__.__name__, "text": self.get_text()}

    def from_dict(self, data: dict) -> ITextComponent:
        self.__text = data["text"]
        return self


class TextDecorator(ITextComponent):
    def __init__(self, text_component: ITextComponent):
        super().__init__()
...
class BoldTextComponent(TextDecorator):
    def __init__(self, text_component: ITextComponent = TextComponent()):
    ...
    def get_text(self) -> str:
        ...
        formatted_text = "\n".join(
            map(
                lambda paragraph: (
                    f"**{paragraph}**" if self._can_decorate(paragraph) else paragraph
                ),
                text.split("\n"),
            )
        )
