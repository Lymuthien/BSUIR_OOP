...
class Document(IDocument):
    ...
    def get_role(self, name: str) -> IUser:
        return self._users.get(name)

    def set_role(self, user: IUser) -> None:
        self._users[user.name] = user
        user.update(f"Your role now: {user.__class__.__name__}")

    def attach(self, observer: IUser) -> None:
        self._users[observer.name] = observer

    def detach(self, observer: IUser) -> None:
    ...
    def notify(self, message: str = None) -> None:
        for observer in self._users.values():
            observer.update(
                f"Document updated: {datetime.datetime.now()}"
                if message is None
                else message
            )
    ...
    def from_dict(self, data: dict) -> "Document":
        ...
        self._components = [
            self._basic_factory.create_text_component(component["text"])
