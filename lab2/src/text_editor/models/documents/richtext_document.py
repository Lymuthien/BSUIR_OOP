...
class MdToRichTextAdapter(RichTextDocument):
    def __init__(self, md_document: MarkdownDocument = MarkdownDocument()):
        super().__init__()
        self.__md_document = md_document
        self._components = [
            self._basic_factory.create_text_component(
                self._convert_md_to_rich(md_document.get_text())
            )
        ]
...
class RichTextToMdAdapter(MarkdownDocument):
    def __init__(self, rich_text_document: RichTextDocument | None = None):
        ...
        if rich_text_document is not None:
            self._components = [
                self._basic_factory.create_text_component(
                    self._convert_rich_to_md(rich_text_document.get_text())
                )
            ]
