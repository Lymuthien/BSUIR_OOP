...
from ...factories.text_component_factory import (
    text_components,
...
class MarkdownDocument(Document):
    ...
    def _apply_style(self, start: int, end: int, factory: type[TextComponentFactory]):
        self._style_factory = factory()

        text = self.get_text()
        before = (
            self._basic_factory.create_text_component(text[:start])
            if start > 0
            else None
        )
        ...
        after = (
            self._basic_factory.create_text_component(text[end + 1 :])
            if end + 1 < len(text)
            else None
        )
        ...
        self._components = [
            component
            for component in (before, style_part, after)
    ...
    def set_theme(self, theme: Theme):
        ...
        self._components = [
            self._basic_factory.create_text_component(
    ...
    def _clear_from_style(self):
        ...
        self._components = [
            self._basic_factory.create_text_component(text_without_style)
        ]
    ...
    def apply_strikethrough(self, start: int, end: int) -> None:
    ...
    def from_dict(self, data: dict) -> "MarkdownDocument":
        ...
        for component_data in data["components"]:
            ...
            component = (
                text_components[type_name]
                .create_text_component()
                .from_dict(component_data)
            )
