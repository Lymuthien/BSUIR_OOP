...
class App:
    ...
    def run(self):
        ...
        coords_warning_msg = (
            f"(x < {self._app.canvas_width}, y < {self._app.canvas_height})"
        )
        msg = ""
        ...
        while True:
            ...
            if main_menu_flag:
                cmd = input(
                    "0 Exit | 1 Draw | 2 Obj selection menu | 3 Save | 4 Load | 5 Undo | 6 Redo\n"
                ).strip()
                ...
                try:
                    if cmd == "0":
                        ...
                        params = input(
                            f"Enter parameters (x, y, width, height)"
                            + coords_warning_msg
                        ).strip()
                        ...
                        params = input(
                            "Enter parameters (x0, y0, x1, y1, x2, y2)"
                            f"(topmost y < {self._app.canvas_height}, "
                            f"leftmost x < {self._app.canvas_width}): "
                        ).strip()
                        params = self._try_to_tuple_int(params.split())
                        if len(params) != 6:
                            raise ValueError("Invalid parameters")
                        ...
                        params = input(
                            "Enter parameters (x, y, vert.rad, hor.rad): "
                            + coords_warning_msg
                        ).strip()
                        params = self._try_to_tuple_int(params.split())
                        if len(params) != 4:
                ...
                try:
                    if cmd == "0":
                        ...
                        params = (
                            input(
                                "Enter parameters (new x, new y): " + coords_warning_msg
                            )
                            .strip()
                            .split()
                        )
                        ...
                        msg = " | ".join(
                            [
                                f"{key}: {value}"
                                for key, value in self._app.get_figure_info().items()
                            ]
                        )
                    elif cmd == "e":
