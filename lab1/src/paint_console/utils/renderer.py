...
class BasicRenderer(IRenderer):
    ...
    def render(
        ...
        for r_idx, row in enumerate(figure_render):
            for s_idx, symbol in enumerate(row):
                if (
                    symbol
                    and (0 <= y + r_idx < len(grid))
                    and (0 <= x + s_idx < len(grid[0]))
                ):
    ...
    def _validate_coordinates(x: int, y: int, grid: list[list[str]]):
        ...
        if y >= len(grid) or (len(grid) > 0 and x >= len(grid[0])):
            raise IndexError(
                "The given figure in that coordinate is out of bounds of grid."
            )

    @staticmethod
    def _check_partial_out_of_bounds(
        figure_render: list[list[str]], x: int, y: int, grid: list[list[str]]
    ):
        """Check for partial out-of-bounds after rendering"""
        if len(figure_render) + y > len(grid) or (
            len(grid) > 0
            and len(figure_render) > 0
            and len(figure_render[0]) + x > len(grid[0])
        ):
