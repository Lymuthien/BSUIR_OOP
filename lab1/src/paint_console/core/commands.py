...
class AddFigureCommand(ICommand):
    def __init__(
        self, model: ICanvasModel, view: ICanvasView, figure: IDrawable, x: int, y: int
    ):
    ...
    def _execute(self):
        self.__figure_id = self.__model.add_figure(
            self.__figure, self.__x, self.__y, layer=self.__layer
        )
    ...
    def redo(self):
        self.__model.add_figure(
            self.__figure,
            self.__x,
            self.__y,
            layer=self.__layer,
            figure_id=self.__figure_id,
        )
        self.__view.update()
...
class RemoveFigureCommand(ICommand):
    def __init__(
        self, model: ISearchingCanvasModel, view: ICanvasView, figure: IDrawable
    ):
...
class MoveFigureCommand(ICommand):
    ...
    def _execute(self):
        self.__model.get_figure_layout(self.__figure_id).coordinates = (
            self.__new_coordinates
        )
        self.__view.update()

    def undo(self):
        self.__model.get_figure_layout(self.__figure_id).coordinates = (
            self.__old_coordinates
        )
        self.__view.update()
...
class ChangeFigureBgCommand(ICommand):
    ...
    def _execute(self):
        self.__model.get_figure_layout(self.__figure_id).figure.background = (
            self.__new_bg
        )
        self.__view.update()
