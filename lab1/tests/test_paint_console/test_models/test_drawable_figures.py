...
class TestDrawableFigure(unittest.TestCase):
    def test_drawable_figure_init_with_empty_bg_raises_error(self):
        with self.assertRaisesRegex(ValueError, 'Background must be a single character'):
...
class TestDrawableEllipse(unittest.TestCase):
    ...
    def test_info_given_correctly(self):
        ...
        with (patch("paint_console.utils.EllipseMath.info", new_callable=PropertyMock) as mock_ellipse_info,
            ...
            mock_drawable_info.return_value = {
                "background": "e"
            }
...
class TestDrawableRectangle(unittest.TestCase):
    ...
    def test_drawable_rectangle_init_with_unvalidated_coords(self, test_name, width, height):
