...
class TestEllipseMath(unittest.TestCase):
    ...
    def test_ellipse_init_invalid_radii_raises_error(self, test_name, v_r, h_r):
        with self.subTest(msg=test_name, v_r=v_r, h_r=h_r):
            with self.assertRaisesRegex(ValueError, "Vertical radius and horizontal radius must be positive"):
                EllipseMath(v_r, h_r)
...
class TestTriangleMath(unittest.TestCase):
    ...
    def test_init_invalid_vertices_raises_error(self, test_name, vertices):
        with self.subTest(msg=test_name, vertices=vertices):
            with self.assertRaisesRegex(ValueError, "All sides must be lower to sum of other sides"):
    ...
    def test_sides_property_returns_correct_value(self):
        self.assertSetEqual(
            set(round(side, 0) for side in self.triangles[0].sides),
            {3, 4, 5}
        )
        self.assertSetEqual(
            set(round(side, 3) for side in self.triangles[1].sides),
            {4, 5.385}
        )
        self.assertSetEqual(
            set(round(side, 3) for side in self.triangles[2].sides),
            {8, 3.606, 5.385}
        )
